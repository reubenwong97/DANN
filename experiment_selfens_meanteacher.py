...
@click.option('--exp', type=click.Choice([
    'visda18_train_val', 'visda18_train_val_red', 'visda18_train_test']), default='visda18_train_val', help='experiment to run')
...
@click.option('--record_history', is_flag=True, default=True, help='Record per-epoch target prediction history')
...
def experiment(exp, arch, rnd_init, img_size, confidence_thresh, teacher_alpha, unsup_weight,
    ...
    import image_dataset, visda18_dataset, visda18_dataset_red
    ...
    with torch.cuda.device(device):
        ...
        if exp == 'visda18_train_val':
            d_source = visda18_dataset.TrainDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = visda18_dataset.ValidationDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif exp == 'visda18_train_val_red':
            d_source = visda18_dataset_red.TrainDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = visda18_dataset_red.ValidationDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif exp == 'visda18_train_test':
            d_source = visda18_dataset.TrainDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = visda18_dataset.TestDataset(img_size=img_shape, range01=True, rgb_order=True)
        ...
        def f_train(X_sup, y_sup, X_unsup0, X_unsup1):
            ...
            #            mask_count]
            outputs = [float(clf_loss.data.item()) * n_samples,
                                float(unsup_loss.data.item()) * n_samples,
                                mask_count]
            # outputs = [float(clf_loss.data) * n_samples,
            #            float(unsup_loss.data) * n_samples,
            #            mask_count]
